...
public static class LandscapeSignatureSampler
{
    ...
    { var q = Local(x, z, seed, s, 20, p.MacroWavelengthBlocks); double a = Gaussian(q.U + .18, q.V - .11, 1, .78); double b = .62 * Gaussian(q.U - .36, q.V + .27, .62, .55); double erosion = .16 * (Noise(q.U * p.MacroWavelengthBlocks / p.DetailWavelengthBlocks, q.V * p.MacroWavelengthBlocks / p.DetailWavelengthBlocks, seed, s, 21) - .5); return Cap(((p.MacroWeight * a) + (p.MesoWeight * b) + (p.DetailWeight * erosion)) * 1.45 - .78); }
    private static double Plateau(double x, double z, int seed, StableId s, LandscapeFamilyProfile p)
    { var q = Local(x, z, seed, s, 30, p.MacroWavelengthBlocks); double r = Math.Pow(Math.Abs(q.U), 4) + Math.Pow(Math.Abs(q.V / .73), 4); double top = 1 - Smooth(.62, 1.06, r); double escarpment = Math.Exp(-42 * (r - 1) * (r - 1)); double interior = .035 * (Noise(q.U * p.MacroWavelengthBlocks / p.DetailWavelengthBlocks, q.V * p.MacroWavelengthBlocks / p.DetailWavelengthBlocks, seed, s, 31) - .5) * Smooth(.25, .85, top); return Cap(-.66 + (1.7 * ((p.MacroWeight * top) + (p.MesoWeight * escarpment) + (p.DetailWeight * interior)))); }
    private static double Basin(double x, double z, int seed, StableId s, LandscapeFamilyProfile p)
    { var q = Local(x, z, seed, s, 40, p.MacroWavelengthBlocks); double r = Math.Sqrt((q.U * q.U) + ((q.V / .62) * (q.V / .62))); double bowl = -Math.Exp(-2.9 * r * r); double rim = .30 * Math.Exp(-35 * (r - .92) * (r - .92)); double strata = .07 * Math.Sin(((p.MacroWavelengthBlocks / p.DetailWavelengthBlocks) * r) + Phase(seed, s, 41)) * Math.Exp(-2 * r * r); return Cap(((p.MacroWeight * bowl) + (p.MesoWeight * rim) + (p.DetailWeight * strata)) * 1.45); }
    ...
    { var q = Local(x, z, seed, s, 60, p.MacroWavelengthBlocks); double cone = Cone(q.U + .23, q.V - .08, .48); double secondary = .55 * Cone(q.U - .39, q.V + .29, .27); double r = Math.Sqrt((q.U - .23) * (q.U - .23) + (q.V + .08) * (q.V + .08)); double caldera = (-.52 * Math.Exp(-90 * r * r)) + (.13 * Math.Exp(-160 * (r - .19) * (r - .19))); return Cap(((p.MacroWeight * cone) + (p.MesoWeight * secondary) + (p.DetailWeight * caldera)) * 1.7 - .68); }
