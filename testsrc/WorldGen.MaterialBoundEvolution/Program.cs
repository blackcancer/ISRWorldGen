...
if (args.Length is < 1 or > 2 || (args.Length == 2 && (!int.TryParse(args[1], out int parsed) || parsed is < 64 or > 512 || (parsed & (parsed - 1)) != 0)))
    throw new ArgumentException("Usage: WorldGen.MaterialBoundEvolution <new-output-directory> [side=128; 64..512 power of two]");
int side = args.Length == 2 ? int.Parse(args[1], System.Globalization.CultureInfo.InvariantCulture) : 128;
...
string[] checks = TectonicChecks.Run().Concat(PolarityRegressionChecks.Run()).Concat(MaterialCohortChecks.Run()).Concat(OceanCarrierChecks.Run()).ToArray();
...
foreach (int seed in new[] { -437287116, 73, 20260906 })
{
    ...
    MaterialBoundHistory history = MaterialBoundHistory.Generate(seed, reference, settings);
    ...
    var summary = new
    {
        seed, algorithm = MaterialBoundHistory.AlgorithmId, commit = Environment.GetEnvironmentVariable("GITHUB_SHA") ?? "UNVERIFIED_WORKTREE",
...
{
    status = "PASS_NUMERICAL_ONLY", algorithm = MaterialBoundHistory.AlgorithmId, checksPassed = checks.Length, checks,
