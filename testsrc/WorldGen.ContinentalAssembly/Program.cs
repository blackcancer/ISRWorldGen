...
if (args.Length != 3 || !int.TryParse(args[1], out int side) || side is < 64 or > 512 || (side & (side - 1)) != 0 || !int.TryParse(args[2], out int selectedSeed))
    throw new ArgumentException("Usage: WorldGen.ContinentalAssembly <new-output-directory> <side64..512> <seed>");
...
string[] checks = TectonicChecks.Run().Concat(PolarityRegressionChecks.Run()).Concat(MaterialCohortChecks.Run()).Concat(OceanCarrierChecks.Run()).Concat(AssemblageChecks.Run()).ToArray();
...
foreach (int seed in new[] { selectedSeed })
{
    ...
    MaterialBoundHistory history = MaterialBoundHistory.GenerateWithAssemblage(seed, reference, settings, assemblage);
    ...
    var summary = new
    {
        seed, algorithm = "continental-assembly-history-v1/" + MaterialBoundHistory.AlgorithmId, commit = Environment.GetEnvironmentVariable("GITHUB_SHA") ?? "UNVERIFIED_WORKTREE",
