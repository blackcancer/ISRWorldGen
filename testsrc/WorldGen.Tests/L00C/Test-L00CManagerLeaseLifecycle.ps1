...
[ordered]@{ TestId='L00-C-MANAGER-LEASE-LIFECYCLE'; Status='PASS'; Scope='Executable deterministic seam simulation; no Vintage Story process was launched.'; Cases='immediate-ready-zero-listener;delayed-ready-unregister-before-install;timeout-30s;attempt-limit-600;resolver-fault;api-mismatch;dispose;late-callback;register-failure;unregister-failure;install-failure;duplicate-start;active-pump-rollback-then-reinstall'; Utc=[DateTimeOffset]::UtcNow.ToString('o') } | ConvertTo-Json
