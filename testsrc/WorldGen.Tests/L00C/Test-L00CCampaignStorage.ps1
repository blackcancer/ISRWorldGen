...
try {
    ...
    [ordered]@{ TestId='L00-C-CAMPAIGN-STORAGE-EXECUTABLE-ORACLE'; Status='PASS'; Isolation='prior campaign bytes preserved; each campaign receives unique flat owned names beneath a supplied GamePaths.Saves test directory'; Collision='existing owned filename is refused without overwrite'; Cleanup='runtime-alive and changed-hash cleanup both refuse and preserve files; sealed exact pairs clean only after stopped proof'; Scope='Temporary paths only; no real AppData, Vintage Story process or UI was used.' } | ConvertTo-Json -Compress
}
