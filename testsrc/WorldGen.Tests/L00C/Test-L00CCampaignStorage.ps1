...
try {
    ...
    [ordered]@{ TestId='L00-C-CAMPAIGN-STORAGE-EXECUTABLE-ORACLE'; Status='PASS'; Isolation='prior campaign bytes preserved; each campaign receives unique flat owned names beneath a supplied GamePaths.Saves test directory'; Collision='existing owned filename is refused without overwrite'; AbortRecovery='prepared; primary-create-intent raw primary; primary-created; secondary-create-intent raw secondary; secondary-created; cycling; interrupted deletion resume; every malformed provenance/PID/extra-owned-name refusal preserves bytes'; LegacyRecovery='single pinned run/PID; explicit manifest plus immutable seal; primary-raw-only; generic no-journal path still refuses; live, mismatch, marker, secondary, unexpected entry, tamper, reparse, hash change and replay preserve bytes; stateful stopped proof is rechecked before manifest, seal, intent, delete and cleaned writes; cleanup interruptions resume from intent'; Cleanup='runtime-alive and changed-hash cleanup both refuse and preserve files; sealed exact pairs retain their separate contract and clean only after stopped proof'; Scope='Temporary paths only; no real AppData, Vintage Story process or UI was used.' } | ConvertTo-Json -Compress
}
