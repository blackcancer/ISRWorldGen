...
function Write-LockedBytes([IO.FileStream]$Stream, [byte[]]$Bytes, [byte[]]$Recovery) { try { $Stream.Position = 0; $Stream.SetLength(0); if ($null -ne $TestHook) { & $TestHook 'AfterTruncate' }; $Stream.Write($Bytes, 0, $Bytes.Length); $Stream.Flush($true) } catch { $Stream.Position = 0; $Stream.SetLength(0); $Stream.Write($Recovery, 0, $Recovery.Length); $Stream.Flush($true); throw } }
...
$launch = Get-CanonicalPath (Join-Path $repository 'src\WorldGen.VintageStory\Properties\launchSettings.json'); $user = Get-CanonicalPath (Join-Path $repository 'src\WorldGen.VintageStory\WorldGen.VintageStory.csproj.user'); $lab = Get-CanonicalPath (Join-Path $repository '.local\L00C')
...
try {
    ...
    if ($Action -eq 'Restore') {
        ...
        Write-LockedBytes $userLock $originalUser $currentUser; Write-LockedBytes $launchLock $original $current
    ...
    foreach ($needed in @($lab, (Join-Path $lab '.isrworldgen-lab'))) { if (-not (Test-Path -LiteralPath $needed)) { throw "Required L00-C laboratory input is missing: $needed" } }
    ...
    try {
        Assert-BootstrapSave $attestation; Write-LockedBytes $launchLock $modified $original; Write-LockedBytes $userLock $modifiedUser $originalUser
        ...
        if ((Get-Sha256 (Read-LockedBytes $launchLock)) -eq $meta.ModifiedSha256) { Write-LockedBytes $launchLock $original $modified }
        if ((Get-Sha256 (Read-LockedBytes $userLock)) -eq $meta.ProjectUserSettingsModifiedSha256) { Write-LockedBytes $userLock $originalUser $modifiedUser }
        throw
    }
}
