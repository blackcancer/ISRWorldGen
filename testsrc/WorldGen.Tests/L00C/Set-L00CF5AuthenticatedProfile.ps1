...
param(
    ...
    [string]$LaunchSettingsPath, [string]$LaboratoryRoot, [string]$PackagePath, [string]$BackupDirectory
)
...
if (-not $LaunchSettingsPath) { $LaunchSettingsPath = Join-Path $repository 'src\WorldGen.VintageStory\Properties\launchSettings.json' }
if (-not $LaboratoryRoot) { $LaboratoryRoot = Join-Path $repository '.local\L00C' }
$launchSettings = Get-CanonicalPath $LaunchSettingsPath; $laboratory = Get-CanonicalPath $LaboratoryRoot
Assert-ChildPath $launchSettings $repository 'launchSettings.json'; Assert-ChildPath $laboratory $repository 'Laboratory root'
...
if (-not $PackagePath) { $PackagePath = Join-Path $laboratory 'menu-action-testmod\Debug\isrworldgenl00clab' }
$package = Get-CanonicalPath $PackagePath; Assert-ChildPath $package $laboratory 'Test-mod package'
