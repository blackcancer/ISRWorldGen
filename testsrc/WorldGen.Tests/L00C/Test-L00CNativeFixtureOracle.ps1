...
try {
    ...
    [ordered]@{ TestId='L00-C-NATIVE-FIXTURE-EXECUTABLE-ORACLE'; Status='PASS'; Mapping='StartServerArgs exact 1.22.7 fields and non-null JsonObject/JToken'; Readiness='all ten predicates required; each individual false refused'; LibrarySha256=(Get-FileHash (Join-Path $GamePath 'VintagestoryLib.dll') -Algorithm SHA256).Hash } | ConvertTo-Json
}
