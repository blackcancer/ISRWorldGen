...
Assert-Contains $text 'InstallResolvedLocked(root, resolution.ScreenManager!)' 'Immediate ready handoff'
...
Assert-Contains $text 'finally { listenerId = 0; tick = null; api = null; }' 'Unregister cleanup clears API and delegate'
Assert-Contains $text 'if (!ReferenceEquals(bootstrapLease, lease) || lease.IsTerminal) return;' 'Late callback rejection'
Assert-Contains $text 'DisposeSession(ICoreClientAPI api)' 'Session disposal cleanup'
