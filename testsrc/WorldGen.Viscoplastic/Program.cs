...
if(args.Length!=3)throw new ArgumentException("Usage: NEW_OUTPUT_DIRECTORY SEED control|plastic");
string root=Path.GetFullPath(args[0]),mode=args[2];int seed=int.Parse(args[1],CultureInfo.InvariantCulture);
if(seed is not(-437287116 or 20260906 or 73)||mode is not("control" or "plastic"))throw new ArgumentException("Frozen paired world campaign only.");
...
try
{
    ...
    var options=new StrainWeakeningOptions(plasticity:new PlasticSheetOptions(enabled:mode=="plastic"));
    ...
    Save("strain-moment",result.Memory.Moment,"continental km times accumulated plastic strain");
    ...
    Write("last-mechanics.json",new { time=result.Solves[^1].Time, nativeSide=options.MechanicalSide,
        referenceStep=1000000d/options.MechanicalSide, fields=result.FinalPlasticity });
    Write("manifest.json",new{seed,mode,commit,algorithm=ViscoplasticSheet.AlgorithmId,policy=result.Policy,settings,options,
        ...
        rheology="VISCOUS_PLUS_BINGHAM_OVERSTRESS_NO_ELASTIC_CRACK_OR_TOPOLOGY",oceanSources="EXISTING_EXCHANGE_RULE_UNCHANGED_NOT_RIFT_TOPOLOGY",
