...
{
    var passed = TectonicChecks.Run().Concat(PolarityRegressionChecks.Run()).Concat(MaterialCohortChecks.Run()).Concat(OceanCarrierChecks.Run()).Concat(AssemblageChecks.Run()).Concat(PacketChecks.Run()).ToArray();
    Console.WriteLine($"ALL_CHECKS_PASS={passed.Length}"); return;
}
if (args.Length != 3 || !int.TryParse(args[1], out int side) || side is < 64 or > 512 || (side & (side - 1)) != 0 || !int.TryParse(args[2], out int selectedSeed))
    throw new ArgumentException("Usage: WorldGen.PacketTransport <new-output-directory> <side64..512> <seed>");
...
string[] checks = TectonicChecks.Run().Concat(PolarityRegressionChecks.Run()).Concat(MaterialCohortChecks.Run()).Concat(OceanCarrierChecks.Run()).Concat(AssemblageChecks.Run()).Concat(PacketChecks.Run()).ToArray();
...
foreach (int seed in new[] { selectedSeed })
{
    ...
    var control = MaterialBoundHistory.GenerateWithDonorControl(seed, reference, settings, assemblage);
    MaterialBoundHistory history = MaterialBoundHistory.GenerateWithLimitedPackets(seed, reference, settings, assemblage);
    if (control.Initial.Checksum != history.Initial.Checksum || control.Ledger.Count != history.Ledger.Count) throw new ArithmeticException("Unmatched numerical control.");
    ...
    WriteField("elevation-model", history.Final.ElevationKm, "signed km model; original native field, not inverted from Y");
    WriteField("initial-elevation-model", history.Initial.ElevationKm, "signed km model; original native initial field");
    WriteField("baseline-height", control.Final.ElevationKm.Select(v => 168 + 12 * v).ToArray(), "donor control with SAME dt and initial material; exact Y");
    WriteField("baseline-elevation-model", control.Final.ElevationKm, "native signed km; matched-step donor control");
    ...
    var summary = new
    {
        seed, algorithm = "carrier-reconstruction-history-v1/" + MaterialBoundHistory.AlgorithmId, commit = Environment.GetEnvironmentVariable("GITHUB_SHA") ?? "UNVERIFIED_WORKTREE",
        ...
        scope = "EXPERIMENTAL_MATERIAL_ATTACHED_KINEMATICS_NOT_FORCE_BALANCED", geographicAcceptance = "NOT_ACCEPTED_PENDING_REFERENCE_REVIEW",
        ...
        blocksPerModelKm = 12, width = side, height = side, settings, scaleChecks, fields, history.TransportPolicy,
        ...
        matchedStepControl = new { control.Checksum, control.TransportPolicy, initialChecksum = control.Initial.Checksum, steps = control.Ledger.Count - 1, initial = control.Ledger[0], final = control.Ledger[^1] },
        solverNotes = new[] { "Limited minmod carrier reconstruction with SSP-RK2; constant donor concentrations, no second-order age claim.",
    ...
    File.WriteAllText(Path.Combine(seedRoot, "control-ledger.json"), JsonSerializer.Serialize(control.Ledger, json));
...
{
    status = "PASS_NUMERICAL_ONLY", algorithm = ContinentalAssemblage.AlgorithmId, checksPassed = checks.Length, checks,
