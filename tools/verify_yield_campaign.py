...
def verify(root,output):
    ...
    for seed in (-437287116,20260906,73):
        for mode in ('ductile','yield'):
            pair=[]
            for os in ('ubuntu-latest','windows-latest'):
                paths=list(root.glob(f'yield-world-{os}-{mode}-{seed}-*'))
                ...
                if m['memoryKind']!='PlasticExcess':raise ValueError('Wrong memory semantics')
                ...
                if mode=='ductile' and any(values['plastic-strain']):raise ValueError('Plastic strain below yielding')
        ...
        a,b=worlds[(seed,'ductile')],worlds[(seed,'yield')]
        ...
        if max(abs(v) for v in delta)<=1e-9:raise ValueError('Yield law did not affect terrain')
        records.append({'seed':seed,'scope':'SAME_INITIAL_STATE_DUCTILE_VS_REGULARIZED_YIELD',
